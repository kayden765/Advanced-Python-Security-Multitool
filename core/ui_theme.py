"""UI theme engine for the MAINFRAME terminal multitool.

Houses the dynamic banner themes used by the `customize` command. Themes are
persisted in the project's shared `core/config.json` (same file that stores the
active `language`) under the `ui_theme` key, so selection is remembered across
sessions and existing config keys are preserved on write.

Exposes:
  * load_theme()    - read the persisted theme key (default "1").
  * save_theme(key) - merge the theme key into core/config.json.
  * handle_customize(current) - interactive theme picker (returns new/old key).
  * show_theme(key) - print the banner art for a theme key.
"""
import json
import os
import time
...
# Cyberpunk / Synthwave Theme Colors
...
# --- MAIN MENU BANNER ART (UI 14 TO 20) ---
...
# Per-UI directory rendering styles (applied in mainframe's directory menu).
DIR_THEME_STYLES = {
    "1":  {"plain": True},
...
# Directory banner rendering - delegates to core.dir_banners module
try:
...
def render_directory_ui(theme_key, dir_id, items=None):
    """Render a themed directory screen using pre-built static banners.

    Each theme has its own banner with the full command list embedded.
    Theme 1 (default) uses plain [NN] format.

    Args:
        theme_key: The active theme key string (e.g. "2", "16").
        dir_id: Directory ID string ("01"-"05").
        items: Unused - kept for backwards compatibility.
    """
    ...
    # Fallback: plain rendering if dir_banners module is unavailable
...
def load_theme():
    """Read the persisted theme key from core/config.json (default '1')."""
    try:
...
def save_theme(theme_key):
    """Persist the theme key into core/config.json, preserving other keys."""
    try:
        data = {}
...
def show_theme(theme_key):
    """Print the banner art for a theme key."""
    _, banner_text, _ = UI_MAP.get(theme_key, UI_MAP["1"])
    print(banner_text)


def handle_customize(current_ui_key):
    """Interactive theme picker. Returns the active key (new or unchanged)."""

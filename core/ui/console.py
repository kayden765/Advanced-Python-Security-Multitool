"""Rich-powered console UI for the MAINFRAME terminal multitool.

Exposes:
  * Colors    - ANSI escape-code palette (always importable; used by the ANSI
                fallback path in mainframe.py when rich is unavailable).
  * ConsoleUI - high-level rich renderer: banner, menus, section headers,
                result/key-value tables, styled panels, prompts, and status
                messages.
  * get_ui()  - returns a process-wide singleton ConsoleUI instance.
"""
...
try:
    ...
    _HAS_RICH = True
except Exception:  # pragma: no cover - rich is a hard dep here
    _HAS_RICH = False


class Colors:
    """ANSI escape-code palette (mirrors the fallback in mainframe.py)."""
    RED = '\033[91m'
    AMBER = '\033[93m'
    YELLOW = '\033[93m'
    GREEN = '\033[92m'
...
class ConsoleUI:
    """High-level, cyberpunk-styled terminal renderer backed by `rich`."""
    ...
    @staticmethod
    def _cell(value):
        """Normalize a value into a renderable cell, parsing markup safely."""
    ...
    def directory_menu(self, title, items, header_color="cyan"):
        """Render a directory/submenu in the same `[NN] Name - Description`
        two-column style as the main `help`/`tools` matrix (no navigation frame
        block, no trailing rule)."""
...
def get_ui():
    """Return a process-wide ConsoleUI singleton (or None if rich is missing)."""
...
# Module-level convenience handle mirroring `ui = get_ui()` in mainframe.py.
